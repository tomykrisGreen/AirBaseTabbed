        ...
        RecyclerView.Adapter<StoryAdapter.ViewHolder>()
{
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            if (holder.adapterPosition === 0)
            {
                myStories(holder.addStory_text!!, holder.story_plus_btn!!, true)  // false changed to true
            }
            else
            {
    ...
    private fun userInfo(viewHolder: ViewHolder, uid: String, position: Int)
    {
        val usersRef = FirebaseDatabase.getInstance().getReference().child("Users").child(uid)
            ...
            override fun onDataChange(p0: DataSnapshot) {
                ...
                if (p0.exists())
                {
                    ...
                    if (position != 0)
                    {
                        ...
                        viewHolder.story_username!!.text = user!!.getName()
                    }
                }
            }
    ...
    private fun seenStory(viewHolder: ViewHolder, uid: String)
    {
        val storyRef = FirebaseDatabase.getInstance().reference
                .child("Story").child(uid)
